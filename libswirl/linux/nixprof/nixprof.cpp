...
void prof_handler (int sn, siginfo_t * si, void *ctxr)
{
	...
	int thd=-1;
	if (pthread_self()==_thread[0]) thd=0;
	else if (pthread_self()==_thread[1]) thd=1;
	else return;
...
static void* profiler_main(void *ptr)
{
	...
	sprintf(line, "/%d.reprof", tick_count);
		...
		do
		{
			...
			for (int i = 0; i < 2; i++) pthread_kill(_thread[i], SIGPROF);
			// printf("Sent SIGPROF\n");
			usleep(prof_wait);
